...
- Step 2: Clone the project repository:
    ...
    1. Change to a working directory you would like to use for this project. Because you will be using a VCL VM, you want to store your files on some storage that is permanent and accessible from other VM instances. You can use [ncsudrive](https://ncsu.service-now.com/sp?id=kb_article_view&sysparm_article=KB0022140). It is already mounted on your VCL VM: `/mnt/ncsudrive/__first_letter_of_your_unity_id__/__your_unity_id__/`. If your unity ID is `xinurocks`, your ncsudrive is mounted at `/mnt/ncsudrive/x/xinurocks`. 
        ```shell
...
1. Preparation
    ...
    - Make sure your output follows the output template shown below, as much as possible. For task 2, the exact values of the base, limit, and pointer do not need to be identical, but they should be reasonably similar. Significant divergence may be a sign of an incorrect implementation. For task 3, as you can see in the sample output, some functions not called in the sample programing appear, such as freemem and kill. This normal as these are functions Xinu utilizes in process management. If your output does not contain them, this is also normal since they might not have been called before `syscallsummary_stop`. If a significant number of other functions are appearing, this may be a sign of an incorrect implementation, and you may want to ask the instructors if your output is acceptable.
        ```
        Beginning provided tests:
        ...
        Process [pid:49]
                Syscall: sys_resume, count: 1, average execution time: 1 (ms)
        ...
        Process [pid:48]
                ...
                Syscall: sys_sleep, count: 1, average execution time: 5001 (ms)
...
2. Submission
    ...
    - Then, commit and push:
        ...
        git push
        ```
        You can do commit and push whenever you want, but the final submission must be committed/pushed with the message `Done`. For a final check, go to your repository using a web browser, and check if your changes have been pushed. If you have already committed all relevant files, you can make an empty commit to add the `Done` message by using the command `git commit --allow-empty -m "Done"`.

## 6. FAQ

- Q) In task 3, do we need to account for pid reuse?
...
- Q) In task 3, I'm getting zero for many of the function times. Is this normal?
    - A) Depending on the function this is reasonable. Many of the functions can take under a millisecond meaning `ctr1000` may not have incremented. We recommend trying functions that are likely to take longer, such as `sleep` and `stacktrace` to verify your implementation.
